...
export default function VideoCard({ data }) {
  return (
    <NavLink
      to={`/video/${data._id}`}
      className="w-full lg:h-96 md:h-80 h-64 overflow-hidden relative rounded-lg bg-slate-700/80 shadow-xl border border-slate-700"
    >
      <LazyLoadImage
        loading="lazy"
        effect="blur"
      ...
      <span className="bg-slate-700/70 absolute top-0 inline-block px-2 py-1 text-xs ml-3 mt-3 rounded font-medium">
        <EyeIcon className={'h-4 w-4 mr-0.5 inline-block'} /> {simplifyNumber(data.views)}
      </span>
