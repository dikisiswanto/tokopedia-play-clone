...
export default function Home() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const { data } = await getVideos({ sortField: activeTab, page, limit: 10, query: searchKeyword});
            ...
            <InfiniteScroll
              ...
              loader={
                <div className="mx-auto col-span-2 md:col-span-3 lg:col-span-5 row-span-1">
                  <PlayCircleIcon className="animate-pulse inline-flex rounded-full bg-green-500 opacity-10" size={48} />
                </div>
              }
              ...
              {page === 1 && loading && (
                [...Array(5)].map((_e, i) => (
                  <div key={i} className="w-full lg:h-96 md:h-80 h-64 overflow-hidden relative rounded-lg bg-slate-700/80 shadow-xl animate-pulse" />
                ))
              )}
